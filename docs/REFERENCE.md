...
**Reasoning Hint** — A distilled textual annotation written by the model during macro compilation. Captures the outcome of a reasoning step at a branch point, not the full chain-of-thought that produced it. Serves three purposes: provenance (explains why a branch was taken during audit), context injection (passed to the RPU as a sliding window of recent decisions when new reasoning is needed), and future learning (gives macro discovery semantic context to work with). Parameterized so concrete values become variables. Immutable once the macro is validated. Contingent — only captured when the repeating subgraph includes branch points where reasoning occurred.
